...
const projects = {
  "pe-license-coverage-dashboard": {
    index:"01", category:"DASHBOARDS", analyticsCategory:"Analytics & BI", title:"PE License Coverage Dashboard", subtitle:"Turning professional engineering license records into a clear view of renewal risk and geographic coverage.", tools:["Power BI","Data Modeling","Data Cleaning","Dashboard Design","Data Visualization"],
  ...
  "digital-marketing-dashboard": {
    index:"02", category:"DASHBOARDS", analyticsCategory:"Analytics & BI", title:"Digital Marketing Performance Hub", subtitle:"Preserving historical performance while marketing reporting moved between platforms.", tools:["Power BI","SharePoint","Data Modeling","Marketing Analytics","Reporting Automation"],
  ...
  "max-verstappen-performance-analysis": {
    index:"04", category:"PERSONAL EXPLORATIONS", analyticsCategory:"Personal Explorations", title:"Max Verstappen Performance Analysis", subtitle:"Exploring Formula 1 performance trends through my first Power BI dashboard.", tools:["Power BI","Data Analysis","Data Visualization","Dashboard Design","Sports Analytics","Data Storytelling"],
  ...
  "asana-marketing-operations": {
    index:"03", category:"SYSTEMS & OPERATIONS", analyticsCategory:"Systems & Operations", title:"Marketing Operations System", subtitle:"Building a clearer operating system for marketing work in Asana.", tools:["Asana","Workflow Design","Process Improvement","Automation Rules"],
  ...
  "quality-content-series": {
    index:"05", category:"MARKETING & CONTENT", analyticsCategory:"Marketing & Content", title:"Quality Is a Mindset", subtitle:"Translating construction quality practices into clear, useful industry content.", tools:["Technical Writing","SEO","Research","Content Strategy"],
  ...
  "marketing-analytics-reporting-suite": {
    index:"06", category:"ANALYTICS & BI", analyticsCategory:"Analytics & BI", title:"Marketing Analytics Reporting Suite", subtitle:"A collection of Power BI dashboards built around different marketing questions and reporting needs.", tools:["Power BI","Marketing Analytics","KPI Reporting","Dashboard Design"],
...
export default async function ProjectPage({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const p=projects[slug as keyof typeof projects];if(!p)notFound();return <main><ProjectViewTracker projectName={p.title} projectSlug={slug} projectCategory={p.analyticsCategory} projectIndex={p.index}/><SiteHeader emailLocation="project_header"/><article className="case">
  ...
  <figure className={`case-image wrap ${"confidentiality" in p||"contain" in p?"contain":""}`}><ImageLightbox src={p.heroImage} alt={p.heroAlt} projectName={p.title} projectSlug={slug} projectCategory={p.analyticsCategory} imageContext="hero" imageName={p.title}/><figcaption><span>{"organization" in p?p.organization:"CARROLL DANIEL"}</span><span>{p.category} / {p.index}</span></figcaption></figure>
  <section className="case-body wrap"><div className="case-block"><p className="section-index">THE QUESTION</p><h2>{p.challenge}</h2></div><div className="case-block approach"><p className="section-index">THE APPROACH</p><div>{p.approach.map((x,i)=><article key={x}><span>0{i+1}</span><p>{x}</p></article>)}</div></div>{"gallery" in p&&<div className="project-gallery">{p.gallery.map(image=><figure key={image.src}><ImageLightbox src={image.src} alt={image.alt} projectName={p.title} projectSlug={slug} projectCategory={p.analyticsCategory} imageContext="gallery" imageName={image.label}/><figcaption>{image.label}</figcaption></figure>)}</div>}<div className="case-block result-block"><p className="section-index">THE RESULT</p><h2>{p.outcome}</h2></div><div className="takeaway"><p className="section-index">WHAT IT TAUGHT ME</p><blockquote>{p.takeaway}</blockquote></div>{"articleUrl" in p&&<a className="published-link" href={p.articleUrl} target="_blank" rel="noreferrer">Read the published article <ArrowUpRight size={18}/></a>}</section>
  </article><CinematicFooter location="project_footer"/></main>}
